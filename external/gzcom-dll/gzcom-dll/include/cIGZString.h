...
{
	public:
		// SC4 was built with MSVC. MSVC places overloaded virtuals in REVERSE
		// declaration order within each overload group in the vtable. GCC/MinGW
		// uses strict declaration order. To produce a vtable that matches
		// SC4's expectations regardless of compiler, we declare overload groups
		// in reverse order when building with MinGW, and in natural order
		// when building with MSVC (which will reverse them itself).
#if defined(__MINGW32__) || defined(__MINGW64__) || (defined(__GNUC__) && !defined(_MSC_VER))
